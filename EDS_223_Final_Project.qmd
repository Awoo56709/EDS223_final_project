...
tm_shape(eez_sf_out) +
  tm_polygons("suitable_km2",
              palette = "blues",  # Accessible cool palette
...
- Southern EEZs (Southern and Central California) show the largest suitable areas.
...
Below is a function that consolidizes the code above into a self-maintained habitat suitability calculator for the U.S West Coast. The parameters needed to use this function include:
...
Enter these values in the folowing order separated by commas (sst_min, sst_max, depth_min, depth_max, species_name)

Given these parameters, the function will color code the west coast aquaculture regions in terms of suitability.
